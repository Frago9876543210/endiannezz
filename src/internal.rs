...
pub trait HackedPrimitive: Primitive {
	fn write<E: Endian, W: Write>(self, w: W) -> Result<()> {
		E::write(self, w)
	}

	fn read<E: Endian, R: Read>(r: R) -> Result<Self> {
		E::read(r)
	}
}
...
pub trait HackedCanIo: CanIo {
	fn write<E: OnlyCanIo, W: Write>(self, w: W) -> Result<()> {
		CanIo::write(&self, w)
	}

	fn read<E: OnlyCanIo, R: Read>(r: R) -> Result<Self> {
		CanIo::read(r)
	}
}
